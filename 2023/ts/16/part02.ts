...

{
  ...
  const run = (light: Light): number => {
    ...
    const stk: Light[] = [light];
    visited[light.c[0]!]![light.c[1]!]!.right = true;
    while (stk.length > 0) {
      const {
        c: [i, j],
        d,
  ...
  let max = 0;
  for (let i = 0; i < H; ++i) {
    max = Math.max(max, run({ c: [i, 0], d: "right" }));
    max = Math.max(max, run({ c: [i, W - 1], d: "left" }));
  }
  for (let j = 0; j < W; ++j) {
    max = Math.max(max, run({ c: [0, j], d: "down" }));
    max = Math.max(max, run({ c: [H - 1, j], d: "up" }));
  }

  console.log(max); // 8331
}
