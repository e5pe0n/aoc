...

{
  ...
  const MAX_NUM_REDS = 12;
  const MAX_NUM_GREENS = 13;
  const MAX_NUM_BLUES = 14;
  ...
  const sum = games
    .map((game) => {
      ...
      if (
        maxSubset.numBlues > MAX_NUM_BLUES ||
        maxSubset.numGreens > MAX_NUM_GREENS ||
        maxSubset.numReds > MAX_NUM_REDS
      ) {
        return 0;
      }
